    ...
    public void mouseClicked(MouseEvent e) {
        if(e.getSource() == submit){
            ...
            if(!username.getText().matches("[a-zA-Z0-9]{4,16}")){
                showDialog("用户名不符合规则，要求4-16位，数字或大写字母或小写字母");
                return;
            }
            ...
            //5、添加用户
            list.add(new User(username.getText(),password.getText()));
            //6、写入文件
            try(BufferedWriter bw = new BufferedWriter(new FileWriter("src\\UserInfo.txt",true))) {
                StringBuilder sb = new StringBuilder();
                sb.append("username=" + username.getText() + "&password=" + password.getText());
                bw.newLine();
